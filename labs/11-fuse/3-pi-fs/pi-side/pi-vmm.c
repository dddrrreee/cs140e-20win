...
void notmain(void) {
    ...
    while(1) {
		...
		case PI_PUT_CODE:
            //printk("***************************************************************\n");
            //printk("PI: Reprogramming the Pi now...\n");
                
			send_ack(op);
			h.cookie = get_uint32();
			h.hdrsize = get_uint32();
			h.addr = get_uint32();
			h.nbytes = get_uint32();
			crc = get_uint32();
                
            //printk("PI VMM: cookie is: %d\n", h.cookie);
            //printk("PI VMM: hdrsize is: %d\n", h.hdrsize);
            //printk("PI VMM: addr is: %d\n", h.addr);
            //printk("PI VMM: nbytes is: %d\n", h.nbytes);
    	
			for(int i = 0; i < h.nbytes; i++){
				code_buf[i] = get_byte();
			}
			
			for(int j = 0; j < h.nbytes; j++) {
				PUT8(h.addr +  sizeof h + j, code_buf[j]);
			}

			BRANCHTO((uint32_t)(h.addr + sizeof h)); 
			break;
		case PI_PRINT_CHAR:
			printk("***************************************************************\n");
			printk("PI: printing char now\n");
